...
resource "aws_route53_record" "admin_db" {
  zone_id        = var.vpn_hosted_zone_id
  name           = "staff-device-admin-db-${var.short_prefix}.${var.vpn_hosted_zone_domain}"
  type           = "A"
  set_identifier = var.region
...
resource "aws_route53_record" "admin_db_verification" {
  zone_id = var.vpn_hosted_zone_id
  ttl     = 60

  for_each = {
    for dvo in aws_acm_certificate.admin_db.domain_validation_options : dvo.domain_name => {
      name   = dvo.resource_record_name
      record = dvo.resource_record_value
      type   = dvo.resource_record_type
    }
